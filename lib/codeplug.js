	...
	resequence() {
		...
		this._properties.channels.forEach(
			(e, i) => {
				...
				const x = this._properties.channels.splice(i, 1);
		...
		this._properties.zones.forEach(
			(e, i) => {
				...
				if (!e.deleted) return;
				const x = this._properties.zones.splice(i, 1);
		...
		this._properties.scan_lists.forEach(
			(e, i) => {
				...
				if (!e.deleted) return;
				const x = this._properties.scan_lists.splice(i, 1);
				this._properties.channels.forEach(
					(ee) => {
						if (ee.scan_list == (i + 1)) {
							ee.scan_list = 0;
						} else if (ee.scan_list > (i + 1)) {
