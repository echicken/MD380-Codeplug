...
//    return parseInt(
      return '' + ((n>>28)&15) + ((n>>24)&15) + ((n>>20)&15) + ((n>>16)&15) + ((n>>12)&15) + ((n>>8)&15) + ((n>>4)&15) + (n&15);
//    );
}
    ...
    record(buffer, def) {
        ...
        Object.keys(def.data).forEach(
            (e) => {
                ...
                if (!def.data[e].validate(ret[e])) {
                    throw `${e}: invalid value: ${ret[e]}`;
                }
				if (typeof def.data[e].transform == 'function') {
					ret[e] = def.data[e].transform(ret[e]);
				}
            }
        );
        return ret;
