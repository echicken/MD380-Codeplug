...
class Write {
    ...
    value(value, buffer, def) {
        switch (def.type) {
            case 'uint8':
                buffer.writeUInt8(value, def.offset);
                break;
            case 'uint16le':
                buffer.writeUInt16LE(value, def.offset);
                break;
            case 'uint24le':
                this.uint24le(value, buffer, def.offset);
                break;
            case 'bcdt':
                this.bcdt(value, buffer, def.offset);
                break;
            case 'bcdle':
                this.bcdle(value, buffer, def.offset);
                break;
            case 'bcdbe':
                this.bcdbe(value, buffer, def.offset);
                break;
            case 'ascii':
                this.ascii(value, buffer, def);
                break;
            case 'unicode':
                this.unicode(value, buffer, def);
                break;
            case 'bit':
            case 'bitfield':
                this.bit(value, buffer, def.offset, def.bit_offset);
                break;
            default:
                break;
        }
